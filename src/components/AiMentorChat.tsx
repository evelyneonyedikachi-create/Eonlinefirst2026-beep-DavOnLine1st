    ...
    <div className="space-y-6">
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-[#00f2ff] font-mono">{persona.role}</p>
                </div>
              </div>

              <p className="text-[11px] text-[#94a3b8] leading-relaxed line-clamp-2 italic mb-2">
                "{persona.quote}"
              </p>
      ...
      <div className="flex items-center gap-2 border-b border-white/10 pb-3">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold font-mono transition-all cursor-pointer backdrop-blur-md ${
            activeTab === "chat"
      ...
      {activeTab === "chat" && (
        <div className="rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-white/10 p-5 md:p-6 space-y-4 shadow-2xl flex flex-col h-[520px]">
          {/* Chat active mentor indicator */}
          <div className="flex items-center justify-between border-b border-white/10 pb-3 text-xs">
            <div className="flex items-center gap-2 font-mono">
              <span className="text-xl">{activePersona.avatar}</span>
              <div>
                <span className="font-bold text-white block">{activePersona.name}</span>
                <span className="text-[#94a3b8] text-[10px]">{activePersona.role}</span>
              </div>
            </div>

            <div className="text-[11px] font-mono text-[#00f2ff] bg-[#00f2ff]/10 px-2.5 py-1 rounded-lg border border-[#00f2ff]/20">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-white/10">
            {messages.map((msg) => {
              ...
              return (
                ...
                >
                  <div className="w-7 h-7 rounded-lg bg-white/[0.05] border border-white/10 flex items-center justify-center text-xs flex-shrink-0">
                  ...
                  >
                    ...
                    <span className="text-[9px] font-mono opacity-60 block mt-1 text-right">
                      {msg.timestamp}
                    </span>
                  </div>
                </div>
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2 pt-2 border-t border-white/10">
            <input
              ...
              className="flex-1 bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-xs md:text-sm text-white focus:outline-none focus:border-[#00f2ff] placeholder:text-slate-500 font-mono backdrop-blur-md"
            />
            <button
              type="submit"
              disabled={isSending || !inputMessage.trim()}
              className="py-2.5 px-5 rounded-xl bg-[#00f2ff] hover:bg-[#33f5ff] text-[#05070a] font-black text-xs uppercase font-mono tracking-wider flex items-center gap-1.5 shadow-[0_0_15px_rgba(0,242,255,0.35)] transition-all cursor-pointer disabled:opacity-50"
            >
      ...
      {activeTab === "review" && (
        <div className="rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-white/10 p-6 space-y-6 shadow-2xl">
          <div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-xs text-slate-200 font-mono focus:outline-none focus:border-purple-400 leading-relaxed backdrop-blur-md"
            />
            ...
            <div className="flex justify-end">
              <button
                ...
                className="py-2.5 px-6 rounded-xl bg-purple-500 hover:bg-purple-400 text-white font-black text-xs uppercase font-mono tracking-wider flex items-center gap-2 shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all cursor-pointer disabled:opacity-50"
              >
                {isReviewing ? (
                  <>
          ...
          {reviewResult && (
            <div className="p-5 rounded-2xl bg-white/[0.03] backdrop-blur-xl border border-purple-500/40 space-y-4 animate-in slide-in-from-bottom-2 duration-300">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-xl bg-purple-500/20 text-purple-300 font-mono font-black text-lg border border-purple-500/30">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-white text-sm">Approved for Production Build</span>
                  </div>
                </div>

                <div className="text-xs font-mono font-bold px-3 py-1.5 rounded-xl bg-amber-500/15 text-amber-300 border border-amber-500/30">
                  Cool Factor: {reviewResult.coolFactor}
                </div>
              </div>

              <div>
                <span className="text-[10px] font-mono uppercase text-[#00f2ff] font-bold block mb-1">
                  Tech Lead Feedback:
                </span>
        ...
        <div className="rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-amber-500/30 p-6 md:p-8 space-y-6 shadow-2xl">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
            ...
            <div className="flex items-center gap-2">
              <span className="px-3 py-1.5 rounded-xl bg-amber-500/20 text-amber-300 font-mono font-bold text-xs border border-amber-500/40">
              ...
              >
                <RefreshCw className={`w-4 h-4 ${isLoadingChallenge ? "animate-spin" : ""}`} />
              </button>
            </div>
          </div>

          <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-md space-y-3">
            <span className="text-[10px] uppercase font-mono text-[#00f2ff] font-bold">
              Mission Objective:
            </span>
            ...
            <div className="p-3 rounded-xl bg-purple-950/20 border border-purple-500/30 text-xs font-mono text-purple-300">
          ...
          <div className="flex justify-end">
            <button
              ...
              className={`py-3 px-8 rounded-xl font-mono text-xs uppercase font-extrabold tracking-wider transition-all flex items-center gap-2 shadow-lg cursor-pointer ${
                challengeCompleted
