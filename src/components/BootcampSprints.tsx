          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <p className="text-base text-slate-400 font-mono">{sprint.duration}</p>
              </div>

              {/* Mini progress indicator */}
              <div className="w-full bg-black/40 h-1.5 rounded-full mt-3 overflow-hidden border border-white/[0.05]">
                <div
      ...
      <div className="rounded-3xl bg-white/[0.03] backdrop-blur-xl border border-white/10 p-6 md:p-8 space-y-6 shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-5">
          <div className="flex items-start gap-4">
            <div className="p-3.5 rounded-2xl bg-white/[0.05] border border-white/10 text-[#00f2ff] font-black shadow-lg">
              {getSprintIcon(activeSprint.sprintNumber)}
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-base font-mono font-bold text-[#00f2ff] uppercase tracking-wider">
          ...
          <div className="p-4 rounded-2xl bg-black/40 border border-white/10 backdrop-blur-md min-w-[220px]">
            <div className="flex justify-between text-base font-mono mb-1.5">
              <span className="text-slate-400">Sprint Mastery:</span>
              <span className="text-[#00f2ff] font-bold">{sprintProgressPct}%</span>
            </div>
            <div className="w-full bg-black/60 h-2.5 rounded-full overflow-hidden border border-white/[0.05]">
              <div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-md">
            <div className="text-base font-mono uppercase text-[#00f2ff] font-bold mb-1.5">
          ...
          <div className="p-5 rounded-2xl bg-amber-500/[0.08] border border-amber-500/30 backdrop-blur-md">
            <div className="text-base font-mono uppercase text-amber-400 font-bold mb-1.5 flex items-center gap-2">
        ...
        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-md space-y-3">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2.5 font-mono text-base text-[#00f2ff] font-bold uppercase">
              <Code2 className="w-5 h-5 text-[#00f2ff]" />
              <span>Capstone Project: {activeSprint.project.name}</span>
            </div>
            <span className="text-base font-mono px-3 py-1 rounded-lg bg-[#00f2ff]/15 text-[#00f2ff] border border-[#00f2ff]/30">
              REAL CODE DELIVERABLE
            </span>
          </div>

          <p className="text-base text-slate-300 leading-relaxed">{activeSprint.project.summary}</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-base font-mono pt-3 border-t border-white/10">
            <div>
              <span className="text-slate-400 block text-base uppercase mb-1">Exact Deliverable:</span>
              <span className="text-[#00f2ff] font-semibold">{activeSprint.project.deliverable}</span>
            </div>
            <div>
              <span className="text-slate-400 block text-base uppercase mb-1">Brag Factor:</span>
              ...
              {activeSprint.milestones.map((m) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-start gap-3.5 select-none backdrop-blur-md ${
                      isCompleted
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <span className={`text-base font-bold ${isCompleted ? "text-white line-through opacity-80" : "text-[#e0e6ed]"}`}>
                          {m.title}
                        </span>
                        <span className="text-base font-mono font-bold px-2.5 py-0.5 rounded-lg bg-[#00f2ff]/10 text-[#00f2ff] border border-[#00f2ff]/20">
                          +{m.xp} XP
                        </span>
                      </div>
                      <p className="text-base text-slate-400 mt-1 leading-relaxed">{m.description}</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {activeSprint.curriculum.map((mod, idx) => (
                <div key={idx} className="p-4 rounded-2xl bg-white/[0.02] border border-white/10 backdrop-blur-md space-y-2">
                  <div className="font-bold text-white text-base">{mod.module}</div>
                  <p className="text-base text-slate-300 leading-relaxed">{mod.description}</p>
                  <div className="flex flex-wrap gap-2 pt-1">
                    {mod.skills.map((skill, sIdx) => (
                      <span key={sIdx} className="px-2.5 py-1 rounded-lg bg-white/[0.04] text-slate-200 text-base font-mono border border-white/10">
                        {skill}
                      </span>
                    ))}
                  </div>
        ...
        <div className="pt-4 border-t border-white/10 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-base font-mono text-[#00f2ff] hover:text-[#33f5ff] underline cursor-pointer"
            >
