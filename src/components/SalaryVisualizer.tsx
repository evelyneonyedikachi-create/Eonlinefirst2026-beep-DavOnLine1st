...
export const SalaryVisualizer: React.FC<SalaryVisualizerProps> = ({ onSelectCareer }) => {
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-8 relative overflow-hidden shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 relative z-10">
        <div>
          <div className="flex items-center gap-2 text-amber-400 font-mono text-base font-semibold uppercase tracking-wider mb-1.5">
          ...
          <p className="text-slate-300 text-base md:text-lg mt-1.5 max-w-2xl">
        ...
        {/* Filter buttons */}
        <div className="flex items-center gap-2 bg-black/40 p-1.5 rounded-xl border border-white/10 backdrop-blur-md self-start md:self-auto">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-base font-semibold transition-all cursor-pointer ${
      ...
      <div className="space-y-4 relative z-10">
        {CAREER_TRACKS.map((career) => {
          ...
          return (
            <div
              ...
              className="group p-4 rounded-xl bg-white/[0.02] hover:bg-white/[0.06] border border-white/10 hover:border-[#00f2ff]/40 backdrop-blur-md transition-all cursor-pointer shadow-sm"
            >
              <div className="flex flex-wrap items-center justify-between gap-2.5 mb-2.5 text-base">
                <div className="flex items-center gap-2.5">
                  <span className="w-7 h-7 rounded-md bg-white/[0.05] flex items-center justify-center font-mono font-bold text-[#00f2ff] text-base border border-white/10">
                    {career.number}
                  </span>
                  <span className="font-bold text-[#e0e6ed] group-hover:text-[#00f2ff] transition-colors text-base">
                    {career.title}
                  </span>
                  <span className="text-base px-2.5 py-0.5 rounded bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 font-medium">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <ArrowUpRight className="w-4 h-4 text-slate-400 group-hover:text-[#00f2ff] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                </div>
              </div>

              {/* Progress Range Bar */}
              <div className="relative w-full h-3 bg-black/50 rounded-full overflow-hidden border border-white/[0.08]">
        ...
        <div className="pt-4 border-t border-white/10 space-y-3">
          <div className="text-base font-mono uppercase text-slate-300 font-semibold">
            Traditional Job Baselines:
          </div>
          {benchmarkComparisons.map((bench, idx) => {
            const widthPct = Math.round((bench.amount / maxVal) * 100);
            return (
              <div key={idx} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-base text-slate-300">
                <span className="sm:w-80 truncate text-base">{bench.title}:</span>
                <div className="flex-1 h-3 bg-black/50 rounded-full overflow-hidden border border-white/[0.05]">
                  <div className={`h-full ${bench.color} rounded-full`} style={{ width: `${widthPct}%` }} />
                </div>
                <span className="font-mono text-slate-300 font-semibold sm:w-28 text-right text-base">
